...
class PaymentNotificationParser {
    ...
    private companion object {
        ...
        val amountRegex = Regex(
            """(?:¥|￥|人民币|RMB|CNY|金额)\s*($amountNumberPattern)|($amountNumberPattern)\s*元""",
            RegexOption.IGNORE_CASE,
        )
        val merchantRegexes = listOf(
            Regex("""(?:向|给|在)([^，,。]+?)(?:付款|支付|消费|转账)"""),
            Regex("""(?:商户名称|交易商户|商户名|商户|交易对手|交易方|对手户名|收款户名|收款方|对方|付款方|付款账户|付款户名|付款人)[:：]\s*([^，,。]+)"""),
        )
        val merchantLabelRegex = Regex("""(?:商户名称|交易商户|商户名|商户|交易对手|交易方|对手户名|收款户名|收款方|对方|付款方|付款账户|付款户名|付款人)[:：]\s*[^，,。]+""")
        ...
        val paymentKeywords = listOf("支付", "付款", "扣款", "收款", "退款", "消费", "转账", "转入", "入账", "存入", "工资", "交易", "支出", "收入", "到账")
        val incomeKeywords = listOf("收款", "收入", "到账", "退款", "转入", "入账", "存入", "工资")
